...
namespace Stratsys.Apis.v1.ExampleTests.Apis.Kpis
{
    public class KpiServiceUT :BaseTest
    {
        ...
        {
            var kpis = Api.Kpis.Filter(name: nameFilter).Fetch().Result;
        ...
        {
            var kpis = Api.Kpis.Filter(idFilter).Fetch().Result;
            Assert.That(kpis.Count, Is.EqualTo(expectedNumberOfKpis));
        }
        ...
        {
            var kpis = Api.Kpis.Filter(departmentId: departmentFilter).Fetch().Result;
            Assert.That(kpis.Count, Is.EqualTo(expectedNumberOfKpis));
        }
        ...
        {
            var kpis = Api.Kpis.Filter(scorecardId: scorecardFilter).Fetch().Result;
            Assert.That(kpis.Count, Is.EqualTo(expectedNumberOfKpis));
        }
        ...
        {
            var kpis = Api.Kpis.Filter(idFilter, departmentFilter).Fetch().Result;
